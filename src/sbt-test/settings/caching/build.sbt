...
/** Update the attributes of the file passed as an argument to make it readable or not for the
  * current user, without updating the "last modified" attribute nor changing the content of the
  * file.
  *
  * Preventing read access to files that are input of protoc is a cheap way to check whether protoc
  * is actually called (as long as there is no prior content hashing done by sbt-protoc) by
  * asserting on the failure of the next run.
  */
